...
import { LoginError, googleLoginUrl, register } from "@/lib/api";
...
export default function RegisterPage() {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      setError(err instanceof LoginError ? err.message : "登録に失敗しました。");
      setSubmitting(false);
    }
  }
    ...
    <div className="flex justify-center py-8">
      <div className="w-full max-w-sm">
        ...
        <p className="text-sm text-slate-500 mb-6">
          アカウントを作成すると、自分専用のテーマ管理・SBI保有銘柄の登録ができるようになります。
        </p>
        ...
        <div className="bg-white border border-slate-200 rounded-2xl shadow-sm p-6">
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="mb-6">
              <label className="block text-sm font-medium mb-1 text-slate-700" htmlFor="password_confirmation">
                パスワード（確認）
              </label>
              <input
                className="border border-slate-300 rounded-lg w-full px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"

...
class StockPageController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('stocks.index', compact('stocks', 'themes', 'badges'));
    }
    ...
    public function show(Stock $stock, FinancialStatementSyncer $syncer, StockScoreRecorder $scoreRecorder)
    {
        ...
        // ROE and equity ratio are both percentages but typically differ by
        // 3-5x in magnitude (ROE ~5-20% vs equity ratio ~20-80%); sharing
        // one axis squashes ROE into an almost-flat, hard-to-read line, so
        // each gets its own axis instead.
        $ratioChartConfig = [
            'type' => 'line',
            'data' => [
                'labels' => $labels,
        ...
        return view('stocks.show', compact(
            'stock', 'trendRows', 'cagr', 'cagrYears', 'salesChartConfig', 'profitChartConfig', 'ratioChartConfig',
            'syncError', 'scoreRecord', 'overview'
        ));
    }
    ...
    public function editThemes(Stock $stock)
    {
        ...
        return view('stocks.themes', compact('stock', 'themes'));
    }

    public function updateThemes(Request $request, Stock $stock)
    {
        $stock->themes()->sync($request->input('theme_ids', []));
